          ...
          <div className="relative">
            ...
            <div className="absolute top-2 right-2 flex space-x-2">
              <Button
                size="icon"
                variant="ghost"
                className="bg-background/80 backdrop-blur-sm hover:bg-background opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
          ...
          <CardContent className="p-3">
            <div className="space-y-2">
              ...
              <Button
                ...
                disabled={product.inventory === 0 && !product.variations}
              >
                <ShoppingCart className="h-3 w-3 mr-1" />
                {product.inventory === 0 && !product.variations ? 'Out of Stock' : product.variations ? 'Select Options' : 'Add to Cart'}
              </Button>
            </div>
          </CardContent>
        </Card>
      </Link>
      {isQuickViewOpen && <QuickView product={product} onClose={() => setIsQuickViewOpen(false)} />}
    </>
  );
}
