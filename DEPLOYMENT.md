# Deployment Guide — GoShop (marketplace)

> BismiLLAH Ar-Rahman Ar-Raheem. This document is the developer contract for deploying
> GoShop. It MUST stay accurate for both supported targets.
...
- Build: `npm run build (vite build + apps/api build)`
- Output dir: `dist/ (SPA) + `functions/api/[[path]].ts` → Pages Functions`
- Deploy (Direct Upload, preferred — does not consume the 500 builds/month CI budget):
  `npx wrangler pages deploy dist/ (SPA) + `functions/api/[[path]].ts` → Pages Functions --project-name=goshop-beta`
- Pages **Functions** policy: REQUIRED: the storefront reads `/api/*` served by Pages Functions bridging to lightbase. If `/api` returns the SPA HTML shell, the Functions bundle is missing — fix the deploy, do not ship.
...
- Build: `vite build && cd apps/api && npm run build`
- Start: `node server.mjs (apps/api)`
- Port: `3000 / $PORT` (bind `0.0.0.0`; honor `$PORT` where the host injects one)
- Node: `20+`
...
- Seeds: raw REST against lightbase (27 collections + 84 docs) — see TEST_USERS.md for accounts.
